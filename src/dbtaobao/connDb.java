...
public class connDb {
    ...
    public static void startConn(){
        try{
            ...
            //连接数据库中间件
            try{
                con = DriverManager.getConnection("jdbc:MySQL://localhost:3306/dbtaobao?useUnicode=true&characterEncoding=utf8&serverTimezone=GMT%2B8&useSSL=false","root","123456");
    ...
    //男女买家各个年龄段交易对比
    public static ArrayList relation() throws SQLException{
        ArrayList<String[]> list = new ArrayList();
        startConn();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select score from rebuy group by score asc");
        while(rs.next()){
